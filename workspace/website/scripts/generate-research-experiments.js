#!/usr/bin/env node
/**
 * Generate research experiment entries from test harness configs
 * 
 * Usage: node scripts/generate-research-experiments.js
 * 
 * Reads:
 * - workspace/config/config_tier*.json (test harness configs)
 * - workspace/docs/discoveries/discoveries.json (metadata)
 * - workspace/src/harness/lfm_tiers.py (tier registry)
 * 
 * Writes:
 * - src/data/research-experiments-generated.ts (auto-generated entries)
 */
...
const OUTPUT_PATH = path.join(__dirname, '..', 'src', 'data', 'research-experiments-generated.ts');

// Tier definitions (from lfm_tiers.py)
const TIERS = [
...
/**
 * Map tier test to simulation type
 */
...
function generateExperimentEntry(tierInfo, test, baseParams) {
  ...
  // Ensure id and testId are always present
 ...
 */
function main() {
  console.log('🔬 Generating research experiments from test harness configs...\n');
