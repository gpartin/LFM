...
# Copyright (c) 2025 Greg D. Partin. All rights reserved.# Copyright (c) 2025 Greg D. Partin. All rights reserved.

# Licensed under CC BY-NC-ND 4.0 (Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International).# Licensed under CC BY-NC-ND 4.0 (Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International).

# See LICENSE file in project root for full license text.# See LICENSE file in project root for full license text.

# Commercial use prohibited without explicit written permission.# Commercial use prohibited without explicit written permission.

# Contact: latticefieldmediumresearch@gmail.com# Contact: latticefieldmediumresearch@gmail.com



""""""

Test Cache Manager (runtime module)Test Cache Manager (runtime module)



Same API as the build/scripts version, but placed under src/ so all runtimeSame API as the build/scripts version, but placed under src/ so all runtime

code can import it without sys.path modifications.code can import it without sys.path modifications.
...
class TestCacheManager:class TestCacheManager:

    """Manages test result caching based on dependency hashing"""    """Manages test result caching based on dependency hashing"""

        

    def __init__(self, cache_root: Path, workspace_root: Path):    def __init__(self, cache_root: Path, workspace_root: Path):

        self.cache_root = cache_root        self.cache_root = cache_root

        self.workspace_root = workspace_root        self.workspace_root = workspace_root

        self.cache_index_path = cache_root / 'cache_index.json'        self.cache_index_path = cache_root / 'cache_index.json'

        self.cache_index = self._load_cache_index()        self.cache_index = self._load_cache_index()

        

    def _load_cache_index(self) -> Dict:    def _load_cache_index(self) -> Dict:

        """Load cache index from disk"""        """Load cache index from disk"""

        if self.cache_index_path.exists():        if self.cache_index_path.exists():

            try:            try:

                return json.loads(self.cache_index_path.read_text(encoding='utf-8'))                return json.loads(self.cache_index_path.read_text(encoding='utf-8'))
    ...
    def _save_cache_index(self):    def _save_cache_index(self):
#!/usr/bin/env python3
...
"""
Test Cache Manager (runtime module)
...
class TestCacheManager:
    ...
    def _load_cache_index(self) -> Dict:
            for file_path in sorted(dir_path.rglob(pattern)):            for file_path in sorted(dir_path.rglob(pattern)):
    ...
    def store_test_results(    def store_test_results(
        ...
        config_file: Optional[Path] = None,        config_file: Optional[Path] = None,
        ...
        Store test results in cache.        Store test results in cache.

                

        Args:        Args:

            test_id: Unique test identifier            test_id: Unique test identifier

            results_dir: Directory containing test results to cache            results_dir: Directory containing test results to cache

            config_file: Optional config file path            config_file: Optional config file path

            metadata: Optional metadata dict (pass rate, duration, etc.)            metadata: Optional metadata dict (pass rate, duration, etc.)
        ...
        cache_dir = self.cache_root / test_id        cache_dir = self.cache_root / test_id
        ...
        cached_results = cache_dir / 'results'        cached_results = cache_dir / 'results'

        if cached_results.exists():        if cached_results.exists():

            shutil.rmtree(cached_results)            shutil.rmtree(cached_results)

        shutil.copytree(results_dir, cached_results)        shutil.copytree(results_dir, cached_results)

                

        # Compute and store dependency hashes        # Compute and store dependency hashes

        dependency_hashes = self.compute_test_dependencies_hash(test_id, config_file)        dependency_hashes = self.compute_test_dependencies_hash(test_id, config_file)
        ...
        manifest = {        manifest = {
            ...
            'dependency_hashes': dependency_hashes,            'dependency_hashes': dependency_hashes,
        ...
        manifest_path = cache_dir / 'manifest.json'        manifest_path = cache_dir / 'manifest.json'
        ...
        # Update cache index        # Update cache index

        self.cache_index[test_id] = {        self.cache_index[test_id] = {

            'test_id': test_id,            'test_id': test_id,

            'cached_at': manifest['cached_at'],            'cached_at': manifest['cached_at'],
        ...
        self._save_cache_index()        self._save_cache_index()

        

    def invalidate_cache(self, test_id: str):    def invalidate_cache(self, test_id: str):

        """Invalidate cached results for a specific test"""        """Invalidate cached results for a specific test"""

        if test_id in self.cache_index:        if test_id in self.cache_index:

            cache_dir = Path(self.cache_index[test_id]['cache_dir'])            cache_dir = Path(self.cache_index[test_id]['cache_dir'])

            if cache_dir.exists():            if cache_dir.exists():

                shutil.rmtree(cache_dir)                shutil.rmtree(cache_dir)

            del self.cache_index[test_id]            del self.cache_index[test_id]
    ...
    def invalidate_all(self):    def invalidate_all(self):
        ...
        if self.cache_root.exists():        if self.cache_root.exists():
    ...
    def clear_cache(self):    def clear_cache(self):
        ...
        """Alias for invalidate_all() - clear entire cache"""        """Alias for invalidate_all() - clear entire cache"""
    ...
    def get_cache_stats(self) -> Dict[str, Any]:    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {        return {
            ...
            'total_cached_tests': len(self.cache_index),            'total_cached_tests': len(self.cache_index),

            'cache_size_mb': self._get_cache_size_mb(),            'cache_size_mb': self._get_cache_size_mb(),

            'cached_tests': list(self.cache_index.keys())            'cached_tests': list(self.cache_index.keys())

        }        }

        

    def _get_cache_size_mb(self) -> float:    def _get_cache_size_mb(self) -> float:

        """Get total cache size in MB"""        """Get total cache size in MB"""

        if not self.cache_root.exists():        if not self.cache_root.exists():
        ...
        for file_path in self.cache_root.rglob('*'):        for file_path in self.cache_root.rglob('*'):
