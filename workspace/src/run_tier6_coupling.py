...
class Tier6Harness(BaseTierHarness):
    ...
    def run_test(self, test_cfg: Dict) -> TestSummary:
        """
        Execute single coupling test.
        
        Args:
            test_cfg: Test configuration from JSON
    ...
    def _run_coup01_relativistic_gravitational_time_dilation(
        self, test_cfg: Dict
    ) -> TestSummary:
        """
        COUP-01: Relativistic + Gravitational Time Dilation
        
        Physics:
            Test that dispersion relation combines SR and GR effects correctly.
            For wave with wavenumber k in χ-field:
            - SR dispersion: ω² = c²k² + m² (m = χ in flat space)
            - With χ-gradient: effective mass varies spatially
            - Expected: ω(χ) ∝ √(c²k² + χ²)
        
        Test Strategy:
            1. Create uniform χ-field (flat space reference)
            2. Measure dispersion relation ω(k) via FFT
            3. Create χ-gradient field
            4. Measure average frequency shift
            5. Compare to prediction: Δω/ω₀ = (√(1 + χ_avg²) - 1)
        
        Success Criteria:
            - Frequency shift matches prediction within 5%
        ...
        log(f"[{test_id}] Grid: {N}³, steps: {steps}, χ={chi_min}->{chi_max}", "INFO")
        
        # Part 1: Reference measurement in flat space (χ = chi_min)
        chi_flat = self.xp.full((N, N, N), chi_min, dtype=np.float64)
        
        # Initialize standing wave with known k
        k_mode = 2 * math.pi / (N * dx / 4)  # 4 wavelengths across domain
        x, y, z = self.xp.meshgrid(
            self.xp.arange(N) * dx, 
            self.xp.arange(N) * dx, 
            self.xp.arange(N) * dx, 
            indexing='ij'
    ...
    def _run_coup07_em_wave_in_chi_gradient(self, test_cfg: Dict) -> TestSummary:
        ...
        # Predicted frequency shift
        chi_avg = (chi_min + chi_max) / 2
        omega_predicted = omega_em * math.sqrt(1 + (chi_max - chi_min)**2)
        
        log(f"[{test_id}] ω_emitted={omega_em:.4f}, ω_predicted={omega_predicted:.4f}", "INFO")
        ...
        # Redshift error
        redshift_error = abs(omega_measured - omega_predicted) / omega_predicted
        
        # Success criteria
        passed = (
            energy_drift < self.tol['energy_drift'] and
            redshift_error < self.tol['frequency_shift_error']
        )
        
        log(f"[{test_id}] ω_measured={omega_measured:.4f}, ω_predicted={omega_predicted:.4f}", "INFO")
        ...
        return TestSummary(
            ...
            convergence_validated=False,
            notes=f"ω_measured={omega_measured:.4f}, ω_predicted={omega_predicted:.4f}"
        )
...
def main():
    ...
    # Initialize harness
    harness = Tier6Harness(cfg, out_root, _default_config_name(), backend=args.backend)
    ...
    log(f"Backend: {args.backend}", "INFO")
