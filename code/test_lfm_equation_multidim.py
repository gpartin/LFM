#!/usr/bin/env python3
"""
LFM Multi-Dimensional Regression — v1.3.5-paritylog
- Uses compensated energy_total.
- Applies guarded energy_lock to BOTH serial and parallel paths BEFORE drift logging.
- Writes per-step parity CSV with serial vs parallel drift (1D quick parity).
"""
...
def run_dim(dim: int, logger):
    ...
    drift_csv = diag_dir / f"parity_{dim}D.csv"
    ...
    for step in range(STEPS):
        ...
        drift_serial_series.append((step+1, (e_now - E0_energy) / (abs(E0_energy)+1e-30)))
    ...
    params["drift_csv_path"] = str(diag_dir / f"parallel_drift_{dim}D.csv")  # optional per-step trace
