...
def load_hamiltonian_data(test_id: str, results_dir: Path = None):
    ...
    if results_dir is None:
        results_dir = Path(__file__).parent / "results" / "Energy"
...
def create_combined_visualization(test_ids: list, output_path: Path = None):
    ...
    if output_path:
        ...
        default_path = Path(__file__).parent / "results" / "Energy" / "hamiltonian_conservation_summary.png"
...
def create_single_test_detail(test_id: str, output_path: Path = None):
    ...
    if output_path:
        ...
        default_path = Path(__file__).parent / "results" / "Energy" / f"{test_id}_hamiltonian_detail.png"
        plt.savefig(default_path, dpi=150, bbox_inches='tight')
        print(f"Saved visualization to: {default_path}")
